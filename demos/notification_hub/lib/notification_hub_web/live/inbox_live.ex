defmodule NotificationHubWeb.InboxLive do
  @moduledoc """
  Unified inbox for notifications streaming in from multiple providers.

  Subscribes to PubSub on mount. Every time a source pushes a new event
  or a user takes an action, a broadcast triggers a re-render — no
  polling, no client-side JavaScript.
  """

  use NotificationHubWeb, :live_view

  alias NotificationHub.Notifications
  ...
  def handle_event("mark_all_read", _params, socket) do
    Notifications.mark_all_read(socket.assigns.filter)
    {:noreply, socket}
  end
  ...
  def handle_info({:notification_added, notification}, socket) do
    socket =
      ...
      |> assign_notifications(Notifications.inbox())

    {:noreply, socket}
  end

  def handle_info(:notifications_changed, socket) do
    {:noreply, assign_notifications(socket, Notifications.inbox())}
  end
  ...
  defp assign_notifications(socket, notifications) do
    ...
    assign(socket,
      ...
      stats: stats,
      sources: Notifications.source_filters()
    )
  end

  defp perform_action("mark_read", id), do: Notifications.mark_read(id)
  defp perform_action("dismiss", id), do: Notifications.dismiss(id)
  defp perform_action("snooze", id), do: Notifications.snooze(id, 15)
  defp perform_action(_other, id), do: Notifications.mark_read(id)
    ...
    <div class="mx-auto max-w-6xl">
      ...
      <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <.icon name={if @show_read, do: "hero-eye-slash", else: "hero-eye"} class="h-4 w-4" />
