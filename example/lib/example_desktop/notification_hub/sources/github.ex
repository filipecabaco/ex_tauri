defmodule ExampleDesktop.NotificationHub.Sources.GitHub do
  @moduledoc """
  Fake GitHub notification source.

  In a real integration this would poll `GET /notifications` on the
  GitHub REST API (or subscribe to a webhook pipeline) and convert each
  event into a `Notification` struct. For the demo we generate a random
  event every few seconds so users can see the hub update in real time.
  """
  ...
  @templates [
    ...
    {"mention", "You were mentioned in #%{num}",
