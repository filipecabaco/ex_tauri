defmodule PomodoroFarmWeb.TimersLive do
  @moduledoc """
  LiveView for the Pomodoro timer farm.

  Every timer on screen is backed by its own supervised GenServer.
  Updates flow back through PubSub so the list re-renders as timers
  tick, pause, reset and complete — all without the LiveView ever
  holding a single timer reference.
  """

  use PomodoroFarmWeb, :live_view
  ...
  def handle_event("start_preset", %{"index" => index}, socket) do
    ...
    if preset, do: Pomodoros.start_timer(%{label: preset.label, duration_s: preset.duration_s})
    {:noreply, socket}
  end
  ...
  def render(assigns) do
    ...
    <div class="mx-auto max-w-5xl">
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Timers</h1>
      ...
      <p class="mt-6 text-center text-xs text-slate-400">
        ...
        Kill one in IEx and the rest keep ticking.
      </p>
    </div>
    """
  end
